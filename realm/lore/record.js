...
function initRecord() {
  ...
  mount.innerHTML = `<nav class="record-breadcrumb"><a href="../">Public Archive</a><span>/</span><span>${record.title}</span></nav>
    ...
    <div class="record-layout"><div>${recordMedia(record)}</div><article class="record-copy"><p class="record-summary">${record.summary}</p><div class="archive-facts">${facts}</div>${body}<section><h2>Known Notes</h2><ul>${notes}</ul></section><section><h2>Canon Source</h2><code>${record.source}</code></section></article></div>
