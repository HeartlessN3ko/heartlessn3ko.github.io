...
function starterAbility(proposal) {
  ...
  return lines.slice(0,7).map((text,index) => ({ key:`ability_${index+1}`, name:String(text).split(/[-—:]/)[0].trim().slice(0,80) || `Ability ${index+1}`, slot:slotFor(index), action_kind:index===0?'passive':'attack', attack_stat:'precision', range:index===0?0:6, accuracy:0, cooldown_turns:index===0?0:index===1?1:index===lines.length-1?0:3, limited_use:index===lines.length-1, effects:index===0?[]:[{kind:'damage',die:index===lines.length-1?12:6,damage_type:'energy'}] }));
}
...
queueEl.addEventListener('click', async event => {
  ...
  try {
    if (button.classList.contains('approve')) {
      ...
      await tacticalFetch('admin-approve', { proposal_id:proposalId, class_name:card.querySelector('[name="class_name"]').value, stats, abilities:JSON.parse(card.querySelector('[name="abilities_json"]').value), approved_equipment:JSON.parse(card.querySelector('[name="equipment_json"]').value), reviewer_notes:notes });
